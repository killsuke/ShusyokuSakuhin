...
public:
	...
	void SetAtk(const int atk) { m_Atk = atk; } // 攻撃力を設定
	void SetHp(const int hp) { m_Hp = hp; } // ヒットポイントを設定
	void SetMaxHp(const int maxHp) { m_MaxHp = maxHp; } // 最大ヒットポイントを設定
	void SetDamage(const int damage) {
		if (m_InvincibleFlag == false) {
			m_TotalDamage = damage;
		}
	};
	void SetUseInvincible(const bool flag) { m_UseInvincible = flag; }; // 無敵を使うかどうかを設定
	void SetInvincibleFlag(const bool flag) { m_InvincibleFlag = flag; }; // 無敵フラグを設定
	void SetUseDeadFlag(const bool flag) { m_UseDeadFlag = flag; }; // 死亡フラグを設定

	int GetAtk() const { return m_Atk; } // 攻撃力を取得
	int GetHp() const { return m_Hp; } // ヒットポイントを取得
	int GetMaxHp() const { return m_MaxHp; } // 最大ヒットポイントを取得
	int GetTotalDamage() const { return m_TotalDamage; };
	bool GetUseInvincible() const { return m_UseInvincible; }; // 無敵を使うかどうかを取得
	bool GetInvincibleFlag() const { return m_InvincibleFlag; }; // 無敵フラグを取得
	bool GetDeadFlag() const { return m_DeadFlag; }; // 死亡フラグを取得

	void AddAtk(const int atk) { m_Atk += atk; } // 攻撃力を加算
	void AddHp(const int hp) { m_Hp += hp; } // ヒットポイントを加算
