...
namespace {
	...
	constexpr int ATTACK_POWER_FIRST = 3;
	constexpr int ATTACK_POWER_SECOND = 5;
	constexpr int ATTACK_POWER_THIRD = 8;
}
...
void TestSwordActionComponent::Update() {
	...
	if (m_IsAction == true) {
		...
		if (isAroundActive == false) {
			...
			// 念のため一度コライダーをオフ
			ColliderAttackComponent* collider = m_Object->GetComponent<ColliderAttackComponent>();
	...
	// 振っていない
		...
		TransformComponent* holderTrans = m_Holder->GetComponent<TransformComponent>();
		XMFLOAT3 holderPos = holderTrans->GetPosition();
...
void TestSwordActionComponent::CreateSwordEffect() {
	...
	XMFLOAT3 pos = myTrans->GetPosition();
...
void TestSwordActionComponent::ChoiceSlashPattern(const RightLeft& horizontalAxis) {
	...
	case ESwordActionState::SLASH_1ST:
		...
		fighter->SetAtk(ATTACK_POWER_FIRST);
	...
	case ESwordActionState::SLASH_2ND:
		...
		fighter->SetAtk(ATTACK_POWER_SECOND);
	...
	case ESwordActionState::SLASH_3RD:
		...
		fighter->SetAtk(ATTACK_POWER_THIRD);
