...
Mesh::Mesh(const Mesh& other) : m_Vertices(other.m_Vertices),m_Indices(other.m_Indices) {
	...
	// サブセット情報のコピー
	for (const std::unique_ptr<SUBSET>& subsetPtr : other.m_Subset) {
		m_Subset.emplace_back(subsetPtr->Clone());
	}
	// マテリアル情報のコピー
	for (const std::unique_ptr<MATERIAL>& materialPtr : other.m_Materiales) {
		m_Materiales.emplace_back(materialPtr->Clone());
	}
	// テクスチャ情報のコピー
	for (const std::unique_ptr<Texture>& texturePtr : other.m_Textures) {
		if (texturePtr) {
