...
namespace {
	constexpr size_t VERTEX_NUM = 3;
	constexpr size_t INDEX_NUM = 3;
}
...
std::vector<VERTEX_3D> TriangleMesh::CreateMeshVertices() {
	...
	m_Vertices[0].uv = XMFLOAT2(0.0f, 1.0f);	// 左下
	m_Vertices[1].uv = XMFLOAT2(0.5f, 0.0f);	// 真ん中上
	m_Vertices[2].uv = XMFLOAT2(1.0f, 1.0f);	// 右下
