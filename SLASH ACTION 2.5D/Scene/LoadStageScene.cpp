...
LoadStageScene::LoadStageScene() {
	...
	// 空の色決定
	FLOAT clearColor[4] = { 0.4f,0.75f, 0.95f, 1.0f };
	...
	GameObject* playOBJ = nullptr;
	{
		...
		rollingColl->SetOffsetSizeOBB({ 4.0f,0.0f,6.0f });
		...
		collAttack->SetOffsetSizeOBB({ 4.0f,0.0f,6.0f });
...
LoadStageScene::~LoadStageScene() {
	...
	DebugSystem::ClearCameraTarget(); // カメラターゲットのリストをクリア
	DebugSystem::ClearColliderObjects();
}
...
void LoadStageScene::TargetAndScroolCreate() {

	{
		...
		DebugSystem::AddCameraTarget(target1->GetInstanceID());
		DebugSystem::AddCameraTarget(target2->GetInstanceID());
		DebugSystem::AddCameraTarget(target3->GetInstanceID());
		DebugSystem::AddCameraTarget(target4->GetInstanceID());
		DebugSystem::AddCameraTarget(target5->GetInstanceID());
		DebugSystem::AddCameraTarget(target6->GetInstanceID());
		DebugSystem::AddCameraTarget(target7->GetInstanceID());
		DebugSystem::AddCameraTarget(target8->GetInstanceID());
...
void LoadStageScene::BackGroundCreate() {
...

	{
		GameObject* backGround = GameObjectManager::AddObject("backGround", "BackGround");
		TransformComponent* trans = backGround->AddComponent<TransformComponent>();
		trans->SetPosition({ 1180.0f,-250.0f,30.0f });
		trans->SetScale({ 110.0f,90.0f,5.0f });
		RenderTerrainComponent* rend = backGround->AddComponent<RenderTerrainComponent>();
		rend->ChangeTexture("testTerrain.png");
		rend->SetShader("TerrainVS.hlsl", "TerrainPS.hlsl");
