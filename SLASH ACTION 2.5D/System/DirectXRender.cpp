...
void DirectXRender::UnInit() {
	...
	for (int i = 0; i < (int)(EBlendState::MAX_BLENDSTATE); ++i) {
		if (m_BlendState[i]) {  // nullptr チェック
			...
			//m_BlendState[i] = nullptr;  // 解放後にポインタをクリア
		}
	}
...
HRESULT DirectXRender::CreateCameraBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"オブジェクトの描画用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateLineThicknessBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"ライン用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateBlurBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"ブラー用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateHitFlashBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"ダメージフラッシュ用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::LightBufferCreate() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"世界のライト用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::MaterialBufferCreate() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"マテリアル用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateMotionBlurBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"モーションブラー（平行移動）用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"モーションブラー（回転アリ）用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::HPBarConstantBufferCreate() {// コンスタントバッファサイズ
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"HPのUI用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateGlowBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"グロー用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"リング型のグロー用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateTimeBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"時間用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
...
HRESULT DirectXRender::CreateShadowBuffer() {
	...
	if (FAILED(hr)) {
		MessageBoxW(nullptr, L"影用のコンスタントバッファ作成に失敗しました。", L"Error", MB_OK);
		return hr;
	}
