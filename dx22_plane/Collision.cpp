...
namespace Collision
{
	...
	// AABBとAABBの当たり判定
	//==================================
	bool CheckHit_CubeAndCube_IsTrigger3D(AABB p1, AABB p2) {
	...
	// 戻しアリ
	//==================================
	bool CheckHit_CubeAndCube_NoTrigger2D(const AABB& p1, const AABB& p2, DirectX::XMFLOAT3& pos) {
	...
	// 戻しアリ
	//==================================
	bool CheckHit_CubeAndCube_NoTrigger3D(const AABB& p1, const AABB& p2, DirectX::XMFLOAT3& pos) {
	...
	bool CubeAndCubeCheck_OBB(const Vector9& col1, const Vector9& col2) {
		...
		// 回転角をラジアンに変換
		float col1PitchRadians = DirectX::XMConvertToRadians(col1.pitch); // X軸回転
		float col1YawRadians = DirectX::XMConvertToRadians(col1.yaw);     // Y軸回転
		float col1RollRadians = DirectX::XMConvertToRadians(col1.roll);   // Z軸回転

		float col2PitchRadians = DirectX::XMConvertToRadians(col2.pitch); // X軸回転
		float col2YawRadians = DirectX::XMConvertToRadians(col2.yaw);     // Y軸回転
		float col2RollRadians = DirectX::XMConvertToRadians(col2.roll);   // Z軸回転

		// OBB1の設定
		obb1.Center = DirectX::XMFLOAT3(col1.x, col1.y, col1.z);
		obb1.Extents = DirectX::XMFLOAT3(col1.w / 2.0f, col1.h / 2.0f, col1.u / 2.0f);
		...
		// OBB2の設定
		obb2.Center = DirectX::XMFLOAT3(col2.x, col2.y, col2.z);
		obb2.Extents = DirectX::XMFLOAT3(col2.w / 2.0f, col2.h / 2.0f, col2.u / 2.0f);
