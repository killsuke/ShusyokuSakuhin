...
void TestExtrusionJudgeComponent::Update()
{
	...
	for (auto& terrain : terrains) {
		auto terrainColls = terrain->GetComponents<ColliderComponent>();
		if (terrainColls.empty()) {
			continue; // コライダーが存在しない場合はスキップ
		}

		for (auto& collider : terrainColls) {

			Vector3 hitNormal = {};
			if (coll->TestNormal(*collider, *coll, hitNormal)) {
