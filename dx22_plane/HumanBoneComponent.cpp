...
HumanBoneComponent::~HumanBoneComponent()
{
...
void HumanBoneComponent::GPU_Update() {
	...
	*/
	...
	// ボーンコンビネーション行列を計算
	for (int i = 0; i < INCH_WORM_BONE_NUM; ++i) {
		g_combMtx[i] = m_bones[i].offsetMtx * m_bones[i].boneMtx;
	}
...
void HumanBoneComponent::BoneInit() {
...
std::vector<AnimationVertex> HumanBoneComponent::CreateBoneMeshVertices() {
	...
	m_boneVertices.resize(4 * INCH_WORM_BONE_NUM);
	...
	// 頭
	m_boneVertices[8] .position = Vector3(-1.0f, 3.0f, 0.0f);
	m_boneVertices[9] .position = Vector3( 1.0f, 3.0f, 0.0f);
	...
	// 頭
	m_boneVertices[8] .color = Color(0.0f, 1.0f, 0.0f, 1.0f);
	m_boneVertices[9] .color = Color(0.0f, 1.0f, 0.0f, 1.0f);
...
std::vector<unsigned int> HumanBoneComponent::CreateMeshIndices() {
	// インデックス情報生成

	m_indices.resize(6 * INCH_WORM_BONE_NUM);
...
X軸回転行列を求める
--------------------------*/
void HumanBoneComponent::DX11MtxRotationX(float angle, DirectX::XMFLOAT4X4& outmtx) {
...
/*------------------------
  Y軸回転行列を求める
--------------------------*/
void HumanBoneComponent::DX11MtxRotationY(float angle, DirectX::XMFLOAT4X4& outmtx) {
...
/*------------------------
   Z軸回転行列を求める
--------------------------*/
void HumanBoneComponent::DX11MtxRotationZ(float angle, DirectX::XMFLOAT4X4& outmtx) {
...
void HumanBoneComponent::DX11MtxInverse(DirectX::XMFLOAT4X4& ansmtx, const DirectX::XMFLOAT4X4& mtx) {
...
void HumanBoneComponent::DX11MtxIdentity(DirectX::XMFLOAT4X4& mat) {
...
void HumanBoneComponent::DX11MtxTranspose(DirectX::XMFLOAT4X4& outmtx, const DirectX::XMFLOAT4X4& inmtx) {
