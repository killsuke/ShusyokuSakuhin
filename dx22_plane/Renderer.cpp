...
ID3D11Device* RendererSystem::m_Device{}; // Direct3Dデバイス
ID3D11DeviceContext* RendererSystem::m_DeviceContext{}; // デバイスコンテキスト
IDXGISwapChain* RendererSystem::m_SwapChain{}; // スワップチェーン
ID3D11RenderTargetView* RendererSystem::m_RenderTargetView{}; // レンダーターゲットビュー
ID3D11DepthStencilView* RendererSystem::m_DepthStencilView{}; // デプスステンシルビュー

// ===================================
// ここら辺の情報は定数バッファとかに与える用として構造体として作る
ID3D11Buffer* RendererSystem::m_WorldBuffer{}; // ワールド行列
ID3D11Buffer* RendererSystem::m_ViewBuffer{}; // ビュー行列
ID3D11Buffer* RendererSystem::m_ProjectionBuffer{}; // プロジェクション行列

ID3D11Buffer* RendererSystem::m_LightBuffer{};	// ライト設定（平行光源）
ID3D11Buffer* RendererSystem::m_MaterialBuffer{};	// マテリアル設定
ID3D11Buffer* RendererSystem::m_TextureBuffer{};  // ＵＶ設定
...
ID3D11BlendState* RendererSystem::m_BlendState[MAX_BLENDSTATE]; // ブレンドステート配列
ID3D11BlendState* RendererSystem::m_BlendStateATC{}; // 特定のアルファテストとカバレッジ（ATC）用のブレンドステート

DirectX::SimpleMath::Matrix* RendererSystem::view_billboard = nullptr;
...
//=======================================
void RendererSystem::SetATCEnable(bool Enable)
{
...
// ワールド行列を設定
//=======================================
void RendererSystem::SetWorldMatrix(Matrix* WorldMatrix)
{
	Matrix world;
...
// ビュー行列を設定
//=======================================
void RendererSystem::SetViewMatrix(Matrix* ViewMatrix)
{
	Matrix view;
...
// プロジェクション行列を設定
//=======================================
void RendererSystem::SetProjectionMatrix(Matrix* ProjectionMatrix)
{
	Matrix projection;
...
void RendererSystem::SetBillboardMatrix(DirectX::SimpleMath::Matrix* _view_billboard) {
	view_billboard = _view_billboard;
};

DirectX::SimpleMath::Matrix RendererSystem::GetBillboardMatrix() {
	return *view_billboard;
};

// ============================================
// ライトを設定
// ============================================
void RendererSystem::SetLight(LIGHT Light) {
	// ライトの設定をGPU側へ送る
...
// マテリアルを設定
//=============================================
void RendererSystem::SetMaterial(MATERIAL Material) {
...
// ＵＶ情報を設定
// ========================================
void RendererSystem::SetUV(float u, float v, float uw, float vh) {
	// ＵＶの行列作成
...
//頂点シェーダー作成
//=======================================
void RendererSystem::CreateVertexShader(ID3D11VertexShader** VertexShader, ID3D11InputLayout** VertexLayout, const char* FileName)
{
...
//ピクセルシェーダー作成
//=======================================
void RendererSystem::CreatePixelShader(ID3D11PixelShader** PixelShader, const char* FileName)
{
	FILE* file; // ファイルを開くためのポインタ
	long int fsize; // ファイルサイズを格納する変数
	fopen_s(&file, FileName, "rb"); // シェーダーファイルをReadBinaryモードで開く
