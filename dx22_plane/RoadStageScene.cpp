...
//	camera->AddComponent<SpringComponent>();

	{
		...
		auto str = stageRoadCSV->AddComponent<StageRoadCSVComponent>();
		auto teM = stageRoadCSV->AddComponent<TerrainManagerComponent>();
		
		str->RoadStageCSV("Stage1.csv", *stageRoadCSV); // ステージのCSVを読み込む
		teM->CreateTerrain(); // 読み込んだCSVからTerrainを生成
	}
