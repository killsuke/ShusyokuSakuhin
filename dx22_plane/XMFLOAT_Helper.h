...
{
    static_assert(std::is_floating_point_v<Ty>, "This type isn't floating point");

    const Ty diff = (epsilon_num > num) ? (epsilon_num - num) : (num - epsilon_num);
    return diff <= std::numeric_limits<Ty>::epsilon();
}
